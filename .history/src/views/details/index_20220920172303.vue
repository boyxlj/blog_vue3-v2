<template>
  <div class="details" >
    ...
    <div class="right">
      <MarkDown v-loading="detailsLoading" :previewOnly="true" :value="detailData.artContent??''"></MarkDown>
...
import {getArtContentById, getRandomArt, getSubmitCollect, getUpdatePageView,setCollectNum} from "@/api/request";
...
const userId = JSON.parse(localStorage.getItem("userInfo") as string)?.userId as number
...
const updateCollectC = async (params:number)=>{
  ...
  if(params>0){
    ...
    cCount= detailData.collectCount * 1 - 1 < 0 ? 0 :detailData.collectCount * 1 - 1
  }
...
const setComment = async (value:any) => {
  await setCommentNum(route.params.artId, value).finally(() => {
    getArtContent()
  })
}
//更新数据
const updates = (type, value,commentCount) => {
  // console.log(type, value,commentCount)
  if (type == "collect") {
    if (value == "jia") {
      setCollect(collectCount.value * 1 + 1)
    } else if (value == "jian") {
      setCollect(
        collectCount.value * 1 - 1 < 0 ? 0 : collectCount.value * 1 - 1
      )
    }
  } else if (type == "comment") {
    if (value == "jia") {
      setComment(artPcomment.value * 1 + 1)
    } else if (value == "jian") {
      // setComment(artPcomment.value * 1 - 1 < 0 ? 0 : artPcomment.value * 1 - 1)
      setComment(artPcomment.value * 1 - commentCount < 0 ? 0 : artPcomment.value * 1 - commentCount)
    }
  }
}
</script>
