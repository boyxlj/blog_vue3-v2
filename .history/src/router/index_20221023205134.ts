...
router.beforeEach( (to,from,next)=>{
    ...
    if(from.path.includes("/search")) { 
      // if(to.path.includes("/tags") || to.path.includes("/search") || to.path.includes("/details")||to.path.includes("/select")){
      if(to.path.includes("/tags") || to.path.includes("/details")||to.path.includes("/select")){
      }else{
        ElMessage.warning("清空")
