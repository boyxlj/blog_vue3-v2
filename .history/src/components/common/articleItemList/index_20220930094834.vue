<template>
  <div class="item" @click="navigateDetails(item.artId??'')" v-for="item in list" :key="item.artId" >
    <div class="item_left">
      <div class="authOrTime">
        <div class="name"  @click.stop="navigateSelect(item.userId??'')">{{item.name}}</div>
        <div class="time">{{getTimeBefore(item.artDate)}}</div>
        <div class="tag">
          <span @click.stop="navigateTags(items)" v-for="(items,index) in item.tag?.split(',')" :key="index">
            {{items}}
          </span>
        </div>
      </div>
      <div class="title">
<!--      <div class="title animate__pulse animate__bounceIn" >-->
<!--      <div class="title animate__animated animate__headShake" >-->
<!--      <div class="title animate__animated animate__flash" >-->
<!--      <div class="title animate__animated animate__shakeX" >-->
        {{item.artTitle}}
      </div>
      <div class="desc">
        {{item.artIntroduce}}
...
const deleteCollect =async (id:string|number,e:Event)=>{
  ...
  if(isDisable) return ElMessage.warning("error!")
